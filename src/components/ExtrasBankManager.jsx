...
export function ExtrasBankManager({ onClose }) {
  ...
  const [items, setItems] = useState(() => JSON.parse(JSON.stringify(state.extrasBank)));
  ...
  const add = () => setItems(prev => [
    ...prev,
    { id: 'bank_' + Date.now(), label: '', price: 0, description: '' }
  ]);

  const save = () => {
    actions.saveExtrasBank(items);
    showMsg('Extras bank saved');
    onClose();
  };
  ...
  return (
    <Modal onClose={onClose}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>Extras Bank</h3>
      ...
      <p style={{ margin: '0 0 16px', fontSize: 13, color: BRAND.muted, lineHeight: 1.5 }}>
        Manage the shared library of optional extras. Items can be added to any proposal from the builder.
      </p>
          ...
          <div key={item.id} style={{ border: '1px solid ' + BRAND.border, borderRadius: 10, padding: 12 }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
              ...
              <input
                type="number"
                className="input"
                style={{ width: 80, minWidth: 0, flex: '0 0 auto' }}
                value={item.price}
