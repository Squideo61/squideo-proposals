...
function collectPredicted(pending, partners, predictKeys, excludedKeys, predictMonthKey) {
  ...
  for (const r of (p.other || [])) {
    if (!r.id) continue;
    if (predictMonthKey && (r.receivedMonths || []).includes(predictMonthKey)) continue;
...
export function FinanceView({ initialTab = null, onBack, onOpenDeal, onOpenCompany, onOpenPartner }) {
  ...
  // none of the whole-business finance (Income / VAT / Performance / charts).
  const canManage = permissionsInclude(state.session?.permissions, 'finance.manage');
  const pendingOnly = !canManage;
  ...
  useEffect(() => {
    if (!canManage) { setLoading(false); return undefined; }
    let active = true;
    setLoading(true);
    actions.loadFinanceStats(effectiveYear).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [actions, effectiveYear, canManage]);

  // Income ledger is period-scoped — reload when the selected period changes.
  useEffect(() => { if (canManage) actions.loadIncome(periodParam); }, [actions, periodParam, canManage]);
  // Income split by new vs existing customers, same period.
  useEffect(() => { if (canManage) actions.loadCustomerMix(periodParam); }, [actions, periodParam, canManage]);
  ...
  useEffect(() => { if (canManage && isSales) actions.loadSalesFinanceStats(effectiveYear); }, [actions, isSales, effectiveYear, canManage]);
  useEffect(() => { if (canManage && isSales) actions.loadSalesLedger(periodParam); }, [actions, isSales, periodParam, canManage]);
  ...
  // Predicted-payments list follows the shared month picker (defaults to the
  // current month), so you can review a past month's predictions. Unfulfilled
  ...
  useEffect(() => { if (canManage && !state.trend) actions.loadTrend(36); }, [actions, state.trend, canManage]);
  ...
  const refreshFinance = () => {
    ...
    actions.loadPredictedPayments(predictMonthKey); // refresh banked-so-far + the predicted list
  ...
  const customerMix = state.customerMix && state.customerMix.period === periodParam ? state.customerMix : null;
  ...
  const predicted = state.predictedPayments && state.predictedPayments.month === predictMonthKey ? state.predictedPayments : null;
  ...
  const predictCtx = useMemo(() => ({
    ...
    keys: predictKeys,
    // Pending-only users (Project/Production Managers) can predict but not run
    // the manage actions (mark paid / invoice / link / remove) — rows hide those.
    canManage,
    toggle: (item, on) => actions.togglePredictedPayment(predictMonthKey, item.key, on, item.label || null, Number(item.amount) || 0),
    ...
    predictInMonth: (item, month) => actions.predictPaymentInMonth(month, item.key, item.label || null, Number(item.amount) || 0)
      .then((r) => { actions.loadPredictedPayments(predictMonthKey); return r; }),
  }), [predictMonthKey, predictKeys, actions, canManage]);
  ...
  useEffect(() => { if (canManage) actions.loadCashflowTargets(); }, [actions, canManage]);
  ...
  // Pending-only users are locked to the Pending Payments tab.
  const effectiveSection = pendingOnly ? 'pending' : section;
    ...
    <div style={{ padding: isMobile ? '20px 16px' : '40px 24px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {!pendingOnly && (
        <PerformancePanel
          ...
          predictedTotal={predictedTotal} predictedMonthKey={predictMonthKey}
      ...
      {/* Section tabs — Income (or Sales) / Pending Payments / VAT. The period
          ...
          Pending-only users get no tab switcher — just Pending Payments. */}
      {!pendingOnly && (
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <Segmented
          value={section}
          onChange={setSection}
          options={[{ value: 'income', label: firstTab.label }, { value: 'predicted', label: `Predicted ${predictMonthName} Payments` }, { value: 'pending', label: 'Pending Payments' }, { value: 'vat', label: 'VAT & Corp tax' }]}
        />
        {section === 'predicted' ? (
          // Predicted is a monthly view — just a month picker (defaults to this
          // month; earlier months show their own list + are rolled into this one).
          <select
            value={monthKey}
      ...
      {effectiveSection === 'income' && (
        <>
          ...
          {isSales ? (
            ...
            <>
              <CustomerMixCard mix={customerMix} periodLabel={firstTab.view.periodLabel} />
      ...
      {effectiveSection === 'predicted' && (
        <PredictedPaymentsSection
          ...
          onSaveNote={(key, note) => actions.setPredictedPaymentNote(predictMonthKey, key, note)}
          onUnpredict={(key) => actions.togglePredictedPayment(predictMonthKey, key, false)}
          onExclude={(item, excluded) => actions.excludePredictedPayment(predictMonthKey, item.key, excluded, item.label || item.name || null, Number(item.amount) || 0)}
...
function PredictedPaymentsSection({ pending, partners, predictKeys, excludedKeys, rolledKeys, rolledAwayKeys, isPastMonth = false, predictMonthKey, monthName, bankedNet, targets, notes = {}, onSaveNote, onUnpredict, onExclude, onOpenDeal, onOpenCompany, onOpenPartner, actions, onChanged, isMobile }) {
  ...
  const excludedItems = useMemo(() => {
    ...
    return collectPredicted(pending, partners, predictKeys, new Set(), predictMonthKey)
      .filter((it) => it.auto && excludedKeys.has(it.key));
  }, [pending, partners, predictKeys, excludedKeys, predictMonthKey]);
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                        ...
                        {it.rolled && (
                          <span title="Carried over from an earlier month — it didn't come in yet" style={{ fontSize: 9, fontWeight: 700, color: '#B45309', background: '#FFF7ED', padding: '1px 5px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: 0.3, whiteSpace: 'nowrap', flexShrink: 0 }}>Rolled over</span>
                        )}
                      </div>
                      {subtitle && (
                    ...
                    <RowActionsMenu items={[
                      ...
                      // Manual rows un-flag; auto rows (partners / other) and
                      // rolled-over rows (no flag for THIS month) get excluded for
                      // this month instead, so they stop reappearing.
                      (it.auto || it.rolled)
...
function ManualPendingRow({ r, cols, isMobile, variant = 'pending', actions, onPaid, onInvoice, onUninvoice, onLink, onLinkCompany, onEdit, companies, onOpenDeal, onOpenCompany, onArchive, onRemove, onChanged }) {
  ...
  // (records the payment in Xero), not the imported-payment toggle. Pending-only
  // users (Project/Production Managers) get only Open + the predict actions.
  const canManage = predict?.canManage !== false;
  const rowActions = isCompanyInvoice ? [
    canManage && { label: 'Mark paid…', icon: Check, onClick: () => setPayingInvoice(true) },
    canOpen && { label: linkedDeal ? `Open ${dealNoun}` : 'Open customer', icon: ExternalLink, onClick: openLinked },
  ] : [
    canManage && (isInvoicedGroup
...
// Income split by NEW vs EXISTING customers for the period — two figures plus a
// bar showing their share, and a line calling out which is higher and by how
// much. "New" = the customer's first-ever income falls in this period.
function CustomerMixCard({ mix, periodLabel }) {
  if (!mix) {
    return (
      <div style={{ background: 'white', border: '1px solid ' + BRAND.border, borderRadius: 12, padding: 20, marginTop: 4 }}>
        <div style={{ padding: '4px', fontSize: 13, color: BRAND.muted }}>Loading customer mix…</div>
      </div>
    );
  }
  const nw = mix.new || { income: 0, count: 0, pct: 0 };
  const ex = mix.existing || { income: 0, count: 0, pct: 0 };
  const total = mix.total || 0;
  const newPct = total ? Math.round((nw.income / total) * 100) : 0;
  const exPct = 100 - newPct;
  const leaderLabel = mix.leader === 'new' ? 'New' : mix.leader === 'existing' ? 'Existing' : null;
  const NEW_C = '#16A34A';
  const EX_C = BRAND.blue;

  return (
    <div style={{ background: 'white', border: '1px solid ' + BRAND.border, borderRadius: 12, padding: 20, marginTop: 4 }}>
      <h3 style={{ margin: '0 0 4px', fontSize: 13, fontWeight: 700, color: BRAND.muted, textTransform: 'uppercase', letterSpacing: 0.6 }}>
        Income by customer type — {periodLabel}
      </h3>
      <p style={{ margin: '0 0 16px', fontSize: 12, color: BRAND.muted }}>
        Net (ex-VAT) income from first-time vs returning customers this period.
      </p>

      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
        {[{ k: 'new', label: 'New customers', d: nw, c: NEW_C, pct: newPct }, { k: 'ex', label: 'Existing customers', d: ex, c: EX_C, pct: exPct }].map((col) => (
          <div key={col.k} style={{ flex: 1, minWidth: 190, border: '1px solid ' + BRAND.border, borderRadius: 10, padding: '12px 14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 700, color: BRAND.muted }}>
              <span style={{ width: 9, height: 9, borderRadius: '50%', background: col.c, display: 'inline-block' }} /> {col.label}
            </div>
            <div style={{ fontSize: 23, fontWeight: 800, color: BRAND.ink, marginTop: 4 }}>{formatGBP(col.d.income)}</div>
            <div style={{ fontSize: 12, color: BRAND.muted, marginTop: 1 }}>
              {col.pct}% of income · {col.d.count} customer{col.d.count === 1 ? '' : 's'}
            </div>
          </div>
        ))}
      </div>
      ...
      {total > 0 && (
        <>
          <div style={{ display: 'flex', height: 10, borderRadius: 6, overflow: 'hidden', marginTop: 16, background: BRAND.border }}>
            <div style={{ width: newPct + '%', background: NEW_C }} title={`New ${newPct}%`} />
            <div style={{ width: exPct + '%', background: EX_C }} title={`Existing ${exPct}%`} />
          </div>
          <div style={{ marginTop: 10, fontSize: 13, color: BRAND.ink }}>
            {leaderLabel
              ? (mix.deltaPct == null
                  ? <><strong>{leaderLabel}</strong> customers are all of this period's income.</>
                  : <><strong style={{ color: mix.leader === 'new' ? NEW_C : EX_C }}>{leaderLabel}</strong> customers brought in <strong>{mix.deltaPct}% more</strong> than {mix.leader === 'new' ? 'existing' : 'new'} this period.</>)
              : <>New and existing customers brought in the same this period.</>}
          </div>
        </>
      )}
      {mix.unattributed > 0 && (
        <div style={{ marginTop: 8, fontSize: 11.5, color: BRAND.muted }}>
          Excludes {formatGBP(mix.unattributed)} of income not tied to a specific customer (imports, partner fees, recurring).
        </div>
      )}
    </div>
  );
