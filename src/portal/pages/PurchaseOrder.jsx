// "Send us your purchase order" — the PO step's own page. Before the number
// lands it's where they submit it; after, it's where "View" takes them: the
// number we hold and every PO document on file, downloadable. The documents are
// the same ones on the CRM deal's Purchase order card, so a PO the team filed
// from an email shows here too, and one uploaded here lands on that card.
...
import { ArrowLeft, Check, FileText, Receipt } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 13.5, fontWeight: 700, color: BRAND.ink, marginTop: 8 }}>
        {uploading ? 'Uploading…' : 'Upload your PO document'}
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: 13.5, color: BRAND.muted, maxWidth: 620, lineHeight: 1.55 }}>
          We quote your PO number on the invoice so it goes straight through with your finance team.
        </p>
      </div>
        ...
        {received ? (
          <div style={{
            ...
            <div style={{ fontSize: 13, color: BRAND.ink, lineHeight: 1.5, flex: 1, minWidth: 180 }}>
              We have your PO — thank you. It’ll be quoted on your invoice.
            </div>
            <div style={{ fontSize: 15, fontWeight: 800, color: BRAND.ink, whiteSpace: 'nowrap' }}>PO {data.poNumber}</div>
          </div>
        ) : (
          <div>
            <SectionHeading>Your PO number</SectionHeading>
            <form onSubmit={submitNumber} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <input
                className="input"
                required
                maxLength={60}
              ...
              />
              <button className="btn" type="submit" disabled={sending}>{sending ? 'Sending…' : 'Submit PO number'}</button>
            </form>
          </div>
        )}
