...
// Monthly revenue targets for the Business → Performance graph. Used when the
// settings row has no finance_targets yet. Seeded from the owner's "Live Sales
// Sheet" monthly totals; fully editable in-app.
...
// Self-heal for db/migrations/20260603_finance_targets.sql so the column exists
// before any read/write below. Module-cached — runs once per cold start.
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const rows = await sql`SELECT extras_bank, inclusions_bank, notification_recipients, revision_call_url, finance_targets FROM settings WHERE id = 1`;
    const row = rows[0];
  ...
  if (req.method === 'PUT') {
    ...
    const { extrasBank, inclusionsBank, notificationRecipients, revisionCallUrl, financeTargets } = req.body || {};
    await sql`
      UPDATE settings SET
