...
// projects[].periods[].consumption[], where a project may return more than one
// billing period overlapping our window — we only want the latest (current) one
// so the figures match the Neon dashboard / invoice. Each consumption datapoint
// carries a `metrics: [{ metric_name, value }]` array (older shapes put the
// metrics as flat keys on the datapoint — handle both).
