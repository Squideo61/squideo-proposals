    ...
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border:1px solid #E5E9EE;border-radius:12px;overflow:hidden;">
        <tr><td style="padding:24px 28px;border-bottom:1px solid #E5E9EE;">
          <div style="font-size:18px;font-weight:800;color:#0F2A3D;">Squideo <span style="color:#2BB8E6;">Client Portal</span></div>
        </td></tr>
...
export function portalWelcomeHtml({ clientName, projectTitle, inviteUrl }) {
  const inner = `
...
export function portalProjectAddedHtml({ clientName, projectTitle, companyName }) {
  const inner = `
...
export function portalTeamInviteHtml({ inviterName, companyName, inviteUrl }) {
  const inner = `
...
export function portalMagicLinkHtml({ loginUrl }) {
  const inner = `
...
export function portalResetHtml({ resetUrl }) {
  const inner = `
...
export function portalExtraConfirmHtml({ clientName, projectTitle, title, amount, originalAmount }) {
