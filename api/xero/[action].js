...
export default async function handler(req, res) {
  ...
  // --- /api/xero/po ---
  // Client confirms the PO route from SignedBlock / ThankYouView. Creates
  // a Xero Quote with reference 'Pending PO'. Lives here (rather than a
  // separate /api/po/[id] route) to keep us under Vercel's 12-function cap.
  if (action === 'po') {
    ...
    const [existing] = await sql`SELECT xero_quote_id FROM payments WHERE proposal_id = ${proposalId}`;
    ...
    try {
      ...
      await sql`
        ...
        VALUES (${proposalId}, 0, 'po', NOW(), ${'po-' + proposalId}, ${billing.accountsEmail || signed.email || null}, ${quoteId})
        ON CONFLICT (proposal_id) DO UPDATE
